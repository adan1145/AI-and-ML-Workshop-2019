...
class DataFrame():
    ...
    def replace_missing(self,replace_with=0.0):
        ...
        for col in col_list:
            if self.dict_[col][1] == type(0.0):
                for row_no,row in list(enumerate(self.columns[col])):
                    if not(isinstance(row,float)):
                        self.columns[col][row_no] = replace_with
        self.__set_rows_from_columns()
        ...
        for col in cols:
            ...
            for val in unique_v:
                ...
                try:
                    ...
                    print(col + 'not in the df.columns')
            try:
    ...
    def get_rows_slice(self,row_range=None):
        ...
        '''
        return copy.deepcopy(self.data_list[row_range[0]:row_range[1]])
